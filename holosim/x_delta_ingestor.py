...
class XDeltaIngestor:
    ...
    def review_and_append(self, deltas: Dict, human_approved: bool = False) -> Dict:
        ...
        print("Health check:", self.chain.health())  # immediate check
        return result

if __name__ == "__main__":
    ingestor = XDeltaIngestor()
    sample_thread = [
        {"id": "post1", "content": "Test invariant: (Concept + Integration + Evolution)^2"},
        {"id": "post2", "content": "Human anchor Canyon locked 2026-07-07"}
    ]
    deltas = ingestor.extract_deltas(sample_thread, "20260707_test")
    result = ingestor.review_and_append(deltas, human_approved=True)
    print("Test append result:", result)